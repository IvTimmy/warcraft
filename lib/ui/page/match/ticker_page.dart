...
class TickerPage extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => _TickerState();
}

class _TickerState extends State<TickerPage>
    with AutomaticKeepAliveClientMixin<TickerPage> {
  ...
  /// 转换成ListView的Item
  ListItemBuilder _buildItem(Upcoming item, DateTime time) {
    PlayerInfo player1 = PlayerInfo(
      item.player1?.name ?? "",
      item.player1?.country ?? "",
      item.race1 ?? "",
    );
    PlayerInfo player2 = PlayerInfo(
      item.player2?.name ?? "",
      item.player2?.country ?? "",
      item.race2 ?? "",
    );
